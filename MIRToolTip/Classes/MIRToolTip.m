...
@property (assign, nonatomic) AlertPosition alertPosition;
...
+ (instancetype)showCardAlertWithTitle:(NSString *)title
    ...
    MIRToolTip* alert = [[MIRToolTip alloc] initCardAlertWithTitle:title
                                                           message:message
                                                         iconImage:nil
                                                          duration:duration
...
- (void)viewDidLoad {
    ...
    if (_alertPosition == AlertPositionBottom) {
...
- (void)constructAlertCardView {
    ...
    messageLabel.textAlignment = _textAlignment;
...
- (void)forceHideInMain {
    ...
    if (currentAlertArray && [currentAlertArray count] > 0) {
        @synchronized (currentAlertArray) {
            ...
            MIRToolTip* activeAlert = currentAlertArray[0];
...
+ (void)hideAlertAnimated:(BOOL)animated {
    ...
    if (currentAlertArray && [currentAlertArray count] != 0) {
        MIRToolTip* alert = currentAlertArray[0];
...
- (void)configureViewForAlertType:(AlertType)alertType iconImage:(UIImage*)iconImage {
